...
- 镜像：`ghcr.io/aaronyang0628/charge-spot-quest:0.1.14`（另有 `latest`；含前端 UI + API）  
...
用仓库 chart [`charts/charge-spot-quest`](charts/charge-spot-quest) 部署 **UI + API**（同镜像 `0.1.14`）。Ingress `/` 出前端，`/api` 为 API；探针 `/health`、`/readyz`。

> **Agent handoff：** 拉 `0.1.14`/`latest` 后 `helm upgrade`（或 `kubectl -n charge-spot rollout restart deploy/charge-spot-quest`）；打开 Ingress 根路径应是 HTML 应用。钉钉用一等公民 `dingtalk.enabled` + `existingSecret` + `publicBaseUrl`，勿再靠巨型 `extraEnv`。
